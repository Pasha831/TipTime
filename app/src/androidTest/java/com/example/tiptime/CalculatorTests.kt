...
class CalculatorTests {
    ...
    @Test
    fun calculate_15_percent_tip_with_no_round_up() {
        onView(withId(R.id.cost_of_service_edit_text)).perform(typeText("121.00"))
        onView(withId(R.id.option_fifteen_percent)).perform(click())
        onView(withId(R.id.round_up_switch)).perform(click())
        onView(withId(R.id.calculate_button)).perform(click())
        onView(withId(R.id.tip_result)).check(matches(withText(containsString("$18.15"))))
    }
}